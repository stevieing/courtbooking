...
describe Slots::Series do
	...
	# 
	# A definite hard to find bug!!! Crucial.
	# originally I was using the cover method of times to create the slots.
	# This causes lots of things to fail if the Time was stubbed or
	# the slots would not build just after midnight.
	# The reason for this is that the time would have to be on that day.
	# slots should build irrespective of the day. It is only the range
	# of times that count! A major coup to fix this.
	#
	describe '#cover?' do
