module BookingsHelpers
  ...
  def create_peak_hours_bookings(courts, user, date, start_time, slot_time, max_bookings)
     date = (date + 7).beginning_of_week
     time = DateTime.parse("#{date.to_s(:uk)} #{start_time}")
     court_number = courts.first.number
     booking = nil

     1.upto(max_bookings + 1) do |n|
       attributes = {user_id: user.id, court_number: court_number, playing_at: time.to_s(:booking)}
   ...
   def create_valid_bookings(users, opponent, courts, date, slots)
     ...
     1.upto(users.length*2) do |i|
       attributes = {user_id: users[incr].id, court_number: court_number, playing_at: "#{date.to_s(:uk)} #{slots[i]}"}
       if i % 2 == 0
