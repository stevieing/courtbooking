...
describe FormManager, :focus => true do
  ...
  let(:nested_params_invalid) { {"nested_models" => { "1" => {"attr_g" => "g"}, "2" => { "attr_f" => "f", "attr_g" => "g" }}}}
  let(:nested_params_update) { {"nested_models" => { "1" => {"attr_f" => "h", "attr_g" => "i"}, "2" => { "attr_f" => "j", "attr_g" => "k" }}}}
  ...
  describe 'associated_models' do
    ...
    context 'valid' do
      ...
      it { expect(subject).to be_valid }
      it { expect(subject.nested_models.first.attr_f).to eq("f")}
      it { expect(subject.nested_models.last.attr_g).to eq("g")}
    ...
    context 'invalid' do
      ...
      it { expect(subject).to_not be_valid }
      it { expect(subject.nested_models.first.attr_f).to be_nil}
      it { expect(subject.nested_models.last.attr_g).to eq("g")}
  ...
  describe 'update' do
    ...
    context 'nested params' do
      ...
      it { expect(subject.nested_models).to have(2).items }
      it { expect(NestedModel.all).to have(2).items }
      it { expect(subject.nested_models.first.attr_f).to eq("h")}
