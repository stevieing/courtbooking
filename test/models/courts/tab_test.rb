...
class TabTest < ActiveSupport::TestCase
  ...
  test "closures should be added to the table" do
    ...
    tab = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Activity, tab.find("13:30", courts.first.id).cell
    assert_instance_of Table::Cell::Activity, tab.find("13:30", courts.last.id).cell
  ...
  test "events should be added to the table" do
    ...
    tab = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Activity, tab.find("09:00", courts.first.id).cell
    assert_instance_of Table::Cell::Activity, tab.find("09:00", courts.last.id).cell
  ...
  test "bookings should be added to the table" do
    ...
    tab = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Booking, tab.find("08:00", courts.first.id).cell
    assert_equal booking1.players, tab.find("08:00", courts.first.id).cell.text
    assert_instance_of Table::Cell::Booking, tab.find("16:30", courts.last.id).cell
  end
  ...
  test "all empty cells should be filled with new bookings" do
    ...
    assert_instance_of Table::Cell::Booking, tab.find("07:00", courts.first.id).cell
    assert_instance_of Table::Cell::Booking, tab.find("12:00", courts[2].id).cell
    assert_instance_of Table::Cell::Booking, tab.find("17:00", courts.last.id).cell
