...
class TabTest < ActiveSupport::TestCase
  ...
  test "if the court is closed slot should be closed" do
    tab = Courts::Tab.new(date, member, slots)
    assert tab.find("16:30", @court.id).closed?
    assert tab.find("17:00", @court.id).closed?
  end
  ...
  test "closures should be added to the table" do
    ...
    tab = Courts::Tab.new(date, member, slots)
    assert_equal :activity, tab.find("13:30", courts.first.id).type
    assert_equal :activity, tab.find("13:30", courts.last.id).type
    assert_equal closure.description, tab.find("13:30", courts.first.id).text
    assert tab.find("14:00", courts.first.id).blank?
    assert tab.find("14:30", courts.first.id).blank?
    assert tab.find("14:30", courts.last.id).blank?
  end

  test "events should be added to the table" do
    event = create(:event, date_from: date, time_from: "09:00", time_to: "10:30", courts: courts)
    tab = Courts::Tab.new(date, member, slots)
    assert_equal :activity, tab.find("09:00", courts.first.id).type
    assert_equal :activity, tab.find("09:00", courts.last.id).type
    assert_equal event.description, tab.find("09:00", courts.first.id).text
    assert tab.find("09:30", courts.first.id).blank?
    assert tab.find("10:00", courts.first.id).blank?
  end
  ...
  test "bookings should be added to the table" do
    ...
    tab = Courts::Tab.new(date, member, slots)
    assert_equal :booking, tab.find("08:00", courts.first.id).type
    assert_equal booking1.players, tab.find("08:00", courts.first.id).text
    assert_equal :booking, tab.find("16:30", courts.last.id).type
  end
  ...
  test "all empty cells should be filled with new bookings" do
    ...
    assert_equal :booking, tab.find("07:00", courts.first.id).type
    assert_equal :booking, tab.find("12:00", courts[2].id).type
    assert_equal :booking, tab.find("17:00", courts.last.id).type
    assert tab.find("07:00", courts.first.id).link?
    assert_equal booking.link_text, tab.find("07:00", courts.first.id).text
  end
