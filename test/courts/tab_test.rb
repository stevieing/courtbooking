...
class TabTest < ActiveSupport::TestCase
  ...
  test "table header should be the date" do
    table = Courts::Tab.new(date, member, slots)
  ...
  test "closures should be added to the table" do
    ...
    table = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Activity, table.find("13:30", courts.first.id).cell
    assert_instance_of Table::Cell::Activity, table.find("13:30", courts.last.id).cell
  ...
  test "events should be added to the table" do
    ...
    table = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Activity, table.find("09:00", courts.first.id).cell
    assert_instance_of Table::Cell::Activity, table.find("09:00", courts.last.id).cell
  ...
  test "bookings should be added to the table" do
    ...
    table = Courts::Tab.new(date, member, slots)
    assert_instance_of Table::Cell::Booking, table.find("08:00", courts.first.id).cell
    assert_equal booking1.players, table.find("08:00", courts.first.id).cell.text
    assert_instance_of Table::Cell::Booking, table.find("16:30", courts.last.id).cell
  end
  ...
  test "all empty cells should be filled with new bookings" do
    ...
    assert_instance_of Table::Cell::Booking, table.find("07:00", courts.first.id).cell
    assert_instance_of Table::Cell::Booking, table.find("12:00", courts[2].id).cell
    assert_instance_of Table::Cell::Booking, table.find("17:00", courts.last.id).cell
  ...
  test "if the date is tomorrow rows in the future should not be in the past" do
    stub_dates(date, "08:31")
    table = Courts::Tab.new(date+1, member, slots)
