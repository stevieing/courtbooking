  ...
  private
    ...
    def create_table #:nodoc
      Table::Base.new do |table|
        table.heading = date_from.calendar_header(date_from+no_of_days)
        table.add :header, header_row
        dates.in_groups_of(split, false).each_with_index do |group, index|
          table.add index.to_s, new_row(group)
        end
      end
    end
    ...
    def header_row #:nodoc
      Table::Row.new do |row|
        date_from.days_of_week(split-1).each do |day|
          row.add day, Table::Cell::Text.new(day)
        end
      end
    end
